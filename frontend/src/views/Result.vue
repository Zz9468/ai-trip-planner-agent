...
const loadAttractionPhotos = async () => {
  ...
  tripPlan.value.days.forEach(day => {
    day.attractions.forEach(attraction => {
      ...
      const promise = fetch(`${API_BASE_URL}/api/poi/photo?${params.toString()}`)
        .then(res => res.json())
        .then(data => {
          if (data.success && data.data.photo_url) {
            attractionPhotos.value[getAttractionKey(attraction)] = data.data.photo_url
          }
        })
        .catch(err => {
...
const getAttractionKey = (attraction: Attraction): string => {
  return attraction.poi_id || attraction.name
}
...
const getAttractionImage = (attraction: Attraction, index: number): string => {
  ...
  // 如果已加载真实图片,返回真实图片
  if (attractionPhotos.value[key]) {
    return attractionPhotos.value[key]
  }
