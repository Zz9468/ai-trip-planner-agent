      ...
      >
        ...
        <a-form-item v-if="loading">
          <div class="loading-container">
            ...
            <div v-if="progressEvents.length" class="agent-events">
              <div
                v-for="(event, index) in progressEvents.slice(-8)"
                :key="`${event.sequence}-${index}`"
                class="agent-event"
              >
                <span class="agent-name">{{ event.agent }}</span>
                <span class="agent-message">{{ event.message }}</span>
              </div>
            </div>
          </div>
        </a-form-item>
...
import { createTripJob, createTripJobEventSource, getTripJob } from '@/services/api'
import type { TripFormData } from '@/types'
import type { TripJobEvent } from '@/services/api'
...
const progressEvents = ref<TripJobEvent[]>([])
...
const appendProgressEvent = (event: TripJobEvent) => {
  progressEvents.value.push(event)
  loadingStatus.value = `${event.agent}: ${event.message}`
  ...
  const baseProgress = progressByAgent[event.agent] ?? loadingProgress.value
  const completedBoost = event.status.includes('completed') ? 8 : 0
  loadingProgress.value = Math.min(95, Math.max(loadingProgress.value, baseProgress + completedBoost))
}

const waitForTripJob = (jobId: string): Promise<void> => {
  return new Promise((resolve, reject) => {
    if (activeEventSource) {
      activeEventSource.close()
    }

    const eventSource = createTripJobEventSource(jobId)
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      finished = true
      eventSource.close()
      reject(new Error('旅行规划任务执行超时,请稍后重试'))
    }, 360000)
    ...
    eventSource.addEventListener('progress', (rawEvent) => {
      const event = JSON.parse((rawEvent as MessageEvent).data) as TripJobEvent
      appendProgressEvent(event)
    })

    eventSource.addEventListener('done', async (rawEvent) => {
      if (finished) return
      finished = true
      const event = JSON.parse((rawEvent as MessageEvent).data) as TripJobEvent
      appendProgressEvent(event)
      ...
      try {
        const job = await getTripJob(jobId)
        if (job.status !== 'succeeded' || !job.data) {
          throw new Error(job.error || job.message || '旅行规划任务未返回结果')
        }

        sessionStorage.setItem('tripPlan', JSON.stringify(job.data))
    ...
    eventSource.addEventListener('failed', (rawEvent) => {
      if (finished) return
      finished = true
      const event = JSON.parse((rawEvent as MessageEvent).data) as TripJobEvent
      appendProgressEvent(event)
      cleanup()
      reject(new Error(event.message || '旅行规划任务失败'))
    })
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.log('旅行规划任务:', job)
    loadingStatus.value = '任务已创建,正在连接进度流...'
    await waitForTripJob(job.job_id)
  } catch (error: any) {
