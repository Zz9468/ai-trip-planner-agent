...
app = FastAPI(
    ...
    description="基于 LangGraph 工作流的智能旅行规划助手API",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def shutdown_event():
    ...
    try:
        ...
        get_amap_mcp_client().close()
    except Exception as e:
        print(f"[WARN] 关闭MCP会话失败: {e}")

    try:
        from ..services.checkpoint_service import get_checkpoint_service

        get_checkpoint_service().close()
    except Exception as e:
        print(f"[WARN] 关闭LangGraph checkpoint失败: {e}")
