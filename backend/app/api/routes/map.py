...
async def health_check():
    ...
    try:
        # 检查服务是否可用
        service = get_amap_service()
        mcp_client = get_amap_mcp_client()
        
        return {
            "status": "healthy",
            "service": "map-service",
            "amap_service": bool(service),
            "mcp_tools_enabled": mcp_client.enabled,
            "mcp_server_module": mcp_client.server_module,
            "mcp_session": mcp_client.status(),
        }
    except Exception as e:
        raise HTTPException(
            status_code=503,
